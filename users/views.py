...
def RejistuUtilizador(request):
	...
	allEnfermaria = Enfermaria.objects.all()
	if request.method == 'POST':
		...
		username = str(firstname)+str(newid3) 
		password = make_password('password')
		obj2 = User(id=newid3, username=username, password=password,email=email,is_active=False)
		obj2.save()
		newid2 = getjustnewid(UserEnfermaria)
		hashid2 = hash_md5(str(newid2))
		if userType == "ubloodbank":
			obj3 = UserEnfermaria.objects.create(user=obj2,firstname=firstname,lastname=lastname,user_created=request.user,hashed=hashid2)
		elif userType == "uenfermaria":
			enfermaria = request.POST.get("enfermaria")
			enfermariaData = get_object_or_404(Enfermaria,id=enfermaria)
			obj3 = UserEnfermaria.objects.create(user=obj2,enfermaria=enfermariaData,firstname=firstname,lastname=lastname,user_created=request.user,hashed=hashid2)
		group_user = Group.objects.get(name=str(userType))
...
def UserProfile(request):
	group = request.user.groups.all()[0].name
	profile = get_object_or_404(UserEnfermaria, user=request.user)
	context = {
		'group':group,
		'profile':profile,
...
def KonfirmaPasswordProfile(request):
	if request.method == 'GET':
		...
		profile = get_object_or_404(UserEnfermaria, user=user)
		...
		if page == "userProfile":
			...
			if authentic:
				messages.success(request, f'Your Password is Authenticated!')
				return redirect('UpdateUserProfilePhoto', profile.hashed)
			else:	
				messages.warning(request, f'Your Password is not Authenticated!')
				return redirect('UserProfile')
...
def UpdateUserProfilePhoto(request,hashid):
	group = request.user.groups.all()[0].name
	profile = get_object_or_404(UserEnfermaria, hashed=hashid)
	if request.method == 'POST':
...
def KonfirmaNoManejaPasswordUtilizador(request, hashid):
	userProfile = get_object_or_404(UserEnfermaria,hashed=hashid)
	...
	if request.method == 'POST':
		...
		if password1 == password2:
			...
			return redirect('KonfirmaNoManejaPasswordUtilizador',userProfile.hashed)
	context = {
		'user':user,
